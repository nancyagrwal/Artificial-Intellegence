...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    counts = []
    for k in range(DIGIT_DATUM_HEIGHT):
         c = 1
         s = 0
         for i in range(DIGIT_DATUM_WIDTH):
             pixel = datum.getPixel(i,k)
             if pixel != c:
                 s += 1
                 c = 1- c
         if s > 5:
             # setting the feature data as 1 and 0 respectively
             features["h_{0}_1".format(k)]=1
         else:
             features["h_{0}_1".format(k)]=0
         if s > 1:
             # setting the feature data as 1 and 0 respectively
             features["h_{0}_0".format(k)]=1
         else:
            features["h_{0}_0".format(k)]=0
...
def enhancedPacmanFeatures(state, action):
    ...
    location = pac_state.getPacmanPosition()
    features['numFoodLeft'] = pac_state.getNumFood()
    location_food = pac_state.getFood().asList()
    foodDis = 0
    
    if location_food:
        foodDis = min([util.manhattanDistance(location, food) for food in location_food])
    
    if foodDis:
      foodDis =10.0/foodDis
    else:
      foodDis  
    features['foodDis'] = foodDis

    ghost_location = pac_state.getGhostPositions()
    ghostDist = 0
    if ghost_location:
        ghostDist = min(util.manhattanDistance(loc, location) for loc in ghost_location)
    if ghostDist:
        ghostDist = 10.0/ghostDist
    else:
        ghostDist
    features['ghostDist'] = ghostDist
    return features
