...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
              possible = util.Counter()
              possible[jailpos] = 1.0
        else:
            for currloc in self.legalPositions:
                 #distance = util.manhattanDistance(pacmanPosition , currloc)
                 possible[currloc] =  self.beliefs[currloc] * emissionModel[util.manhattanDistance(pacmanPosition , currloc)] 
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for prev_pos in self.legalPositions:
              gameState = self.setGhostPosition(gameState, prev_pos)
              currPos = self.getPositionDistribution(gameState)
              for newPos, prob in currPos.items():
                  
                     possible[newPos] += self.beliefs[prev_pos] * prob
        self.beliefs = possible
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        particlatxy = self.numParticles / len(legalPos)
        for position in legalPos:
             for k in range(particlatxy):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.particles = [ jailPos for x in xrange (len (self.particles)) ]
        else:
            possible = util.Counter()
        
            for particle in self.particles:
                distance = util.manhattanDistance(pacmanPosition, particle)
                possible[particle] += emissionModel[distance]
            ...
            if  possible.totalCount()  == 0:
               ...
               key = possible.keys()
               self.particles = util.nSample(value,key , num)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
              pos.add(p)
       # finding old positions :  
        for curr in pos:
            # finding new position distance
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, curr))

            dist[curr] = newPosDist
        for curr in self.particles:
              # getting sample belief distributions
              new.append(util.sample(dist[curr]))
        self.particles = new
    ...
    def getBeliefDistribution(self):
        ...
        belief_dis = util.Counter()
        # converting list of particles to a belief distribution.
        for particle in self.particles:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        possiblePosition = list(itertools.product(legal, repeat = ghosts))
        random.shuffle(possiblePosition)
        partiArray = self.particles = []
        init = 0
        while init < particles:
            for pos in possiblePosition:
                if init < particles:
                    # appending the array for possible actions
                    partiArray.append(pos)
                    init += 1
                else:
                   break
        ...
        for j, p in enumerate(self.particles):
              ...
              for i in range(self.numGhosts):
                  ...
                  if noisyDistances[i] == None:
                      # when the ghost has not been captured by the pacman
                      p = self.getParticleWithGhostInJail(p , i)
                  else:
                      dist *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]
              possible[p] += dist
        ...
        if not any (possible.values()):
             ...
             for j, p in enumerate(particles):
               ...
               for b in range(0, num):
                     # util.sample for belief distribution sample
                     part_array.append(util.sample(possible))
               self.particles = part_array
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for n in range(self.numGhosts):
                 newParticle[n] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), n , self.ghostAgents[n]))
    ...
    def getBeliefDistribution(self):
        ...
        belief_dis = util.Counter()
        # converting lost of particles to a belief distribution.
        for particle in self.particles:
               belief_dis[particle] += 1
        belief_dis.normalize()
        return  belief_dis
